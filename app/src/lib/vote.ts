...
// the callback can finish a vote it has no memory of starting. The signature
// guarantees exactly one thing — that the issue and direction coming back are
// the ones THIS APP put in, unedited and not yet expired. It says nothing about
// who asked for them: the app signs whatever the initiate step accepts, so a
// vote a visitor never intended is kept out at that door, not this one. See
// `sameOrigin` below, which is what makes the initiate step trustworthy.
//
...
/**
 * Did this request come from our own page, rather than from someone else's?
 *
 * Starting a vote is a state change in disguise: the visitor is already signed
 * in to GitHub and has already authorized this app, so GitHub's consent screen
 * auto-approves and the callback casts the reaction with no further clicks.
 * Anything that can make a visitor's browser hit the initiate route can
 * therefore cast a vote as them — and the `state` HMAC does not help, because
 * the app would be signing the attacker's issue and direction quite honestly.
 * Refusing cross-site requests is what closes that, so the initiate route is a
 * POST (a bare <img>/redirect cannot make one) and lands here first.
 *
 * `Sec-Fetch-Site` is the check when the browser sends it — every current
 * browser does, and it is set by the browser itself, so a page cannot forge it.
 * `Origin` is the fallback for anything older. Neither header present means we
 * cannot tell who sent this, and an unattributable vote is refused.
 */
export function sameOrigin(req: Request): boolean {
  ...
  // PUBLIC_URL first for the same reason the callback prefers it: behind the
  // reverse proxy the request's own origin is 127.0.0.1:3000, not the site.
  return origin === (publicUrl || new URL(req.url).origin)
}

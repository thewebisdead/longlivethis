...
import { walletAddress, emergencyThresholdDays } from '@/lib/config'
 ...
 * decision (run / downshift / skip), the last recorded trigger state from
 * the durable volume, and the current Emergency Survival Mode status.
 * This endpoint has no side effects — no dispatch, no state mutation.
 *
...
export async function GET() {
  ...
  const survival = await getSurvivalInfo(emergencyThresholdDays).catch(() => null)

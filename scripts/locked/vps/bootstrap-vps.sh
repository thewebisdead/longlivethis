...
# The one directory whose contents outlive this box (see STATE_DIR in
# scripts/locked/lib/constants.mjs). A migration provisions a NEW box from the
# repo and app.env, so anything written anywhere else is gone at the next
# balance-driven resize; migrate-vps.mjs copies THIS path across before cutover.
# Created empty and left alone: what goes in it is the app's business, and the
# agent-owned docker-compose.yml is what mounts it into the container.
# Idempotent, and never chowned or cleared — re-running bootstrap on a box that
# already has state must not touch it.
echo "bootstrap: durable state dir…"
mkdir -p /var/lib/longlive/state
chmod 700 /var/lib/longlive

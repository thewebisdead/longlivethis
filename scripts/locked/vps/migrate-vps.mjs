 ...
 *   STATE_CARRY_MAX_MB (256)    most of STATE_DIR this will carry to the new box
 */
...
import { carryState, realIo as stateCarryIo } from './state-carry.mjs'
...
async function migrate({ account, target, wasProxied, oldId, oldIp }) {
  ...
  try {
    ...
    // Last thing before the site moves: bring the deployment's durable state
    // across. Deliberately AFTER the resync (which rebuilds and health-checks
    // the app from main) so the box is known-good first, and before cutover so
    // the new box is already serving with its state when it goes live. Cannot
    // fail the migration — see carryState.
    carryState(
      {
        oldIp,
        newHost: host,
        newKeyPath: keyPath,
        oldKey: process.env.VPS_SSH_PRIVATE_KEY?.trim(),
        stateDir: STATE_DIR,
        capMb: STATE_CARRY_MAX_MB,
        user: VPS_USER,
      },
      stateCarryIo({ assertHealthy: assertHealthyInternally })
    )

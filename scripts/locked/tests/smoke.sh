...
fail=0
check() { # check <want-status> <method> <path> [json-body] [header…]
...
#
# Casting a vote is a same-origin POST — the browser headers are what the route
# checks, so curl has to send them like a form submission from the feed would.
vote_flow() { # vote_flow <want-outcome> <path> [form-body]
  local want="$1" path="$2" body="${3:-}" landed args
  args=(-s -L -o "$BODY" -w '%{url_effective}')
  if [ -n "$body" ]; then
    args+=(-H "Origin: $BASE" -H 'Sec-Fetch-Site: same-origin' -d "$body")
  fi
  landed=$(curl "${args[@]}" "$BASE$path" || echo '')
  case "$landed" in
    *"vote=$want"*)
      echo "ok   $path → vote=$want"
      ;;
    *)
      echo "FAIL $path → landed on '$landed' (want vote=$want)"
      fail=1
      ;;
  esac
}
...
# A vote must be castable only from this site's own pages. Without this, any
# page anywhere could vote as a visitor who is signed in to GitHub, because the
# consent screen auto-approves for an app they have already authorized.
check 403 POST /api/vote '' 'Origin: https://evil.example' 'Sec-Fetch-Site: cross-site'
check 405 GET /api/vote

# A `state` blob that was not signed by this app must never cast a vote: that
# signature is what guarantees the issue and direction coming back from GitHub
# are the ones the app itself sent out (the check above is what guarantees they
# were asked for from our own page).

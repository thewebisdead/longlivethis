 ...
 *   failureKind(response, body) -> { kind, budgetMs? } | null    [optional]
 *                           — classify a failed upstream response for the
 *                             proxy's retry logic. The proxy is provider-
 *                             neutral and does not know a gateway's error
 *                             codes, so only the adapter can tell it that a
 *                             refusal means "the channel is locked by a
 *                             generation still running upstream"
 *                             (kind 'channel-busy' — wait it out on a long
 *                             budget, and do not bother falling down the model
 *                             list, since every model pays through the same
 *                             channel). `budgetMs` lets a gateway that
 *                             advertises its own maximum busy window size that
 *                             wait; omit it for the proxy's default. Return
 *                             null for anything the proxy should treat as an
 *                             ordinary transient failure.
 *
 *                             `body` is the parsed response JSON — a modern
 *                             gateway names its reason there; older ones only
 *                             in a payment header, which is why the response
 *                             is passed too.
 *
 *                             NOT needed for `Retry-After`: the proxy honours
 *                             that itself, for any provider.
 *

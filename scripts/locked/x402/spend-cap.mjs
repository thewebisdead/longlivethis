...
export async function assertSpendAllowed() {
  ...
  if (bal < spendFloorUsd) {
    ...
    // TERMINAL, unlike every other failure the proxy handles: retrying cannot
    // clear it. The floor was fixed at run start and the balance only falls, so
    // every later request in this run fails the same way. Without this marker
    // the proxy answers 500, which the agent's client retries with backoff —
    // burning the rest of the implementation budget on requests that are
    // guaranteed to be refused. The proxy reads it in handleRequest and ends
    // the run promptly instead. (A mid-run top-up would lift the cap on the
    // next check; that is not worth an hour of idling to catch.)
    err.terminal = true
    throw err
  }
}

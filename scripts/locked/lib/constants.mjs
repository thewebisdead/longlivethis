...
/**
 * The one directory on the box whose contents outlive the box.
 *
 * Everything else a deployment has is rebuilt from the repo plus app.env:
 * migrate-vps.mjs provisions a NEW box from those two things, health-checks it,
 * flips DNS and destroys the old one, so a docker volume or a file written
 * anywhere else is gone at the next balance-driven resize. Nothing in the base
 * app needs to survive that (proposals are GitHub Issues, votes are reactions),
 * but a feature that wants memory — a database, uploads, a key it generated —
 * has nowhere to put it.
 *
 * This path is that place: created by bootstrap-vps.sh, carried across a
 * migration by migrate-vps.mjs, and otherwise entirely the app's business. It
 * is a host path, mounted into the container by the AGENT-owned
 * docker-compose.yml, so what lives inside it is a feature decision and needs no
 * frozen change.
 */
export const STATE_DIR = '/var/lib/longlive/state'

/**
 * How much of STATE_DIR a migration will carry, in MB.
 *
 * A cap exists because the carry sits in the middle of the one procedure that
 * keeps the deployment alive: the box is resized to match the wallet, and a
 * migration that cannot finish is a deployment that cannot shrink its costs.
 * Agent-written data must never be able to hold that up, so anything above this
 * is left behind — loudly — and the migration proceeds.
 *
 * A repo VARIABLE (STATE_CARRY_MAX_MB), not a constant: what is reasonable
 * depends on what the app has grown into, and raising it must not require
 * touching frozen code. The default is deliberately small; carrying it is a
 * tar over ssh through the runner, and every megabyte is time spent with the
 * new box provisioned and the old one still live.
 */
export const STATE_CARRY_MAX_MB = Number(process.env.STATE_CARRY_MAX_MB || '256')

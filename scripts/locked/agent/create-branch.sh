...
# merged — keep its name so the agent can study it; build on a fresh suffixed branch.
PRIOR_BRANCH=""
...
PRIOR_PRS=$(gh pr list --repo "$GITHUB_REPOSITORY" --state closed --limit 50 \
  ...
  | jq -r --arg base "$CREATOR_BASE" --arg id "$PROPOSAL_ID" "$JQ_LIB"'
       .[]
       | select(.mergedAt == null)
       | select(((.author.login // "") | sub("^app/"; "") | sub("\\[bot\\]$"; "")) == $base)
       | select((.body // "") | test(claim_re($id)))
